...
#try
#1)generate CPU and memory usage reports
#store the reports in a directory
#2)notifies  me by email whenever CPU or memory usage 
#exceeds certain thresholds that you set
...
function generate_report {
	...
	filenum="$(ls ./reports_dir | wc -l)"
	oldest_file=$(ls -t ./reports_dir |tail -1)
	if [[ filenum -ge MAXIMUM_REPORTS ]]; then
		rm ./reports_dir/$oldest_file
	fi
...
function notify
{
	...
	cpu_usage_int=$(printf "%.f" $percentage)
	...
	if [ $(echo $cpu_usage_int '>=' $CPU_THRESHOLD | bc -l) ] || [ $(echo $cpu_usage_int '>=' $MEMORY_THRESHOLD | bc -l) ]; then
		/usr/bin/mailx -s "report" $USER < ./reports_dir/$file_name
